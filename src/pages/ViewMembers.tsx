...
export function ViewMembers({ onSelectMember }: ViewMembersProps) {
  ...
  const counts = {
    ...
    active: members.filter(
      (m) =>
        getMemberStatus(m.memberships[m.memberships.length - 1]) === "active"
    ).length,
    expiring: members.filter(
      (m) =>
        getMemberStatus(m.memberships[m.memberships.length - 1]) === "expiring"
    ).length,
    expired: members.filter(
      (m) =>
        getMemberStatus(m.memberships[m.memberships.length - 1]) === "expired"
    ).length,
  };
          ...
          <div className="space-y-3 max-h-[550px] overflow-y-auto">
            {filteredMembers.map((member) => {
              ...
              const daysLeft = latestMembership
                ? getDaysUntilDue(latestMembership.end_date)
                : null;
